...
int main()
{
	double price1, price2, rate2, rate;
	char again;
	...
	do
	{
		...
		rate2 = inflation(price1, price2);
		rate = (price2 * (rate2 / 100.0)) + price2; 
		cout << "The estimated price one year from now is " << rate << endl << "and the estimated price from two years from now is " << (rate * (rate2 / 100)) + rate << endl;
